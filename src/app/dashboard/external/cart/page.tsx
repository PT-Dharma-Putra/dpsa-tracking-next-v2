...
import { Trash2, ArrowRight, ShoppingBag, Plus, Minus, PackageOpen, Loader2 } from "lucide-react"
...
const CartItem = ({ item, updateQuantity, removeFromCart }: { item: any, updateQuantity: any, removeFromCart: any }) => {
    return (
        <div className="flex flex-col gap-4 py-6 first:pt-0 border-b border-neutral-100 last:border-0 sm:flex-row sm:items-center">
            {/* Image */}
            ...
            <div className="flex flex-1 flex-col justify-between sm:flex-row sm:items-center gap-4">
                <div className="flex-1 space-y-1">
                    <h3 className="font-medium text-neutral-900 line-clamp-2">{item.nama_barang}</h3>
                    ...
                    <p className="text-sm font-medium text-neutral-900 sm:hidden">
                        Rp {(item.harga_jabodetabek || 0).toLocaleString()}
                    </p>
                </div>
                ...
                <div className="flex items-center justify-between gap-6 sm:justify-end">
                    ...
                    <div className="hidden text-right sm:block w-32">
                        ...
                        <p className="font-medium">Rp {(item.harga_jabodetabek || 0).toLocaleString()}</p>
                    </div>
                    ...
                    <div className="flex items-center rounded-lg border border-neutral-200 bg-white shadow-sm">
                        <button
                            ...
                            className="flex h-9 w-9 items-center justify-center text-neutral-500 hover:text-orange-600 disabled:opacity-50"
                            disabled={item.quantity <= 1}
                        >
                            <Minus className="h-3.5 w-3.5" />
                        </button>
                        <div className="flex h-9 w-10 items-center justify-center border-x border-neutral-100 text-sm font-semibold">
                            {item.quantity}
                        </div>
                        <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
                            className="flex h-9 w-9 items-center justify-center text-neutral-500 hover:text-orange-600"
                        >
                    ...
                    <div className="flex items-center gap-4 pl-2">
                        ...
                        <Button
                            size="icon"
                            variant="ghost"
                            className="h-9 w-9 text-neutral-400 hover:bg-red-50 hover:text-red-600 rounded-full"
