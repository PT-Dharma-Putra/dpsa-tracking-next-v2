...
export function KategoriFormDialog({ open, onOpenChange, kategori }: KategoriFormDialogProps) {
    ...
    const mutation = useMutation({
        mutationFn: (data: Partial<KategoriBarang>) => {
            if (kategori) {
                return KategoriBarangService.updateKategori(kategori.id, data)
            }
            return KategoriBarangService.createKategori(data)
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["kategori-barang"] })
            toast.success(kategori ? "Kategori barang berhasil diperbarui" : "Kategori barang berhasil ditambahkan")
            onOpenChange(false)
        },
        onError: (error: any) => {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        <DialogTitle>{kategori ? "Edit Kategori Barang" : "Tambah Kategori Barang"}</DialogTitle>
                        <DialogDescription>
                            {kategori ? "Perbarui informasi kategori barang." : "Tambah kategori barang baru ke sistem."}
                        </DialogDescription>
                    </DialogHeader>
