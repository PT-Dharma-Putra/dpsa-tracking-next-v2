...
import { KategoriBarangService } from "@/features/kategori-barang/services/kategori-service"

interface KategoriImportDialogProps {
    open: boolean
    onOpenChange: (open: boolean) => void
}

export function KategoriImportDialog({ open, onOpenChange }: KategoriImportDialogProps) {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.writeFile(workbook, "Template_Import_Kategori.xlsx")
        toast.success("Template Excel berhasil didownload")
    }

    const mutation = useMutation({
        mutationFn: (items: { nama: string; kode: string }[]) => KategoriBarangService.importKategori(items),
        onSuccess: (data: any) => {
            queryClient.invalidateQueries({ queryKey: ["kategori-barang"] })
            toast.success(data.message || "Data kategori berhasil diimpor")
            onOpenChange(false)
        },
        onError: (error: any) => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                const items = json.map((row) => {
                    const namaVal = row.nama || row.Nama || row["nama kategori"] || row["Nama Kategori"] || ""
                    const kodeVal = row.kode || row.Kode || row["kode kategori"] || row["Kode Kategori"] || ""
                    return {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        <DialogTitle>Import Kategori Barang</DialogTitle>
                        <DialogDescription>
                            Import data kategori barang secara massal menggunakan file Excel (.xlsx atau .xls).
                        </DialogDescription>
                    </DialogHeader>
