...
export function KategoriTable() {
    ...
    const [selectedKategori, setSelectedKategori] = React.useState<KategoriMDL | null>(null)
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = React.useState(false)
    const [kategoriToDelete, setKategoriToDelete] = React.useState<KategoriMDL | null>(null)

    const { data: kategoriResponse, isLoading } = useQuery({
        queryKey: ["kategori-mdl", page, search],
        queryFn: () => KategoriMDLService.getKategori({ page, search }),
    })
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: number) => KategoriMDLService.deleteKategori(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["kategori-mdl"] })
            toast.success("Kategori MDL berhasil dihapus")
            setIsDeleteDialogOpen(false)
            setKategoriToDelete(null)
        },
        onError: () => {
            toast.error("Gagal menghapus kategori MDL")
        }
    })

    const handleEdit = (kategori: KategoriMDL) => {
        setSelectedKategori(kategori)
        setIsFormOpen(true)
    }

    const handleDelete = (kategori: KategoriMDL) => {
    ...
    const handleExportExcel = async () => {
        ...
        setIsExporting(true)
        try {
            const response = await KategoriMDLService.getKategori({ per_page: -1, search })
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, "Kategori MDL")
            ...
            XLSX.writeFile(workbook, "Data_Kategori_MDL.xlsx")
        ...
        <div className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-neutral-800">Daftar Kategori MDL</p>
                ...
                <Table>
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            kategoriList.map((kategori: KategoriMDL, index: number) => (
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Hapus Kategori MDL</AlertDialogTitle>
                        <AlertDialogDescription>
                            Apakah Anda yakin ingin menghapus kategori MDL <strong>"{kategoriToDelete?.nama}"</strong>? Tindakan ini tidak dapat dibatalkan.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
