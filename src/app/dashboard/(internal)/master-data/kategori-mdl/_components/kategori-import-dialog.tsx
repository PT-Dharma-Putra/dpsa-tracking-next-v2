...
export function KategoriImportDialog({ open, onOpenChange }: KategoriImportDialogProps) {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Template Kategori MDL")
        XLSX.writeFile(workbook, "Template_Import_Kategori_MDL.xlsx")
        toast.success("Template Excel berhasil didownload")
    }

    const mutation = useMutation({
        mutationFn: (items: { nama: string; kode: string }[]) => KategoriMDLService.importKategori(items),
        onSuccess: (data: any) => {
            queryClient.invalidateQueries({ queryKey: ["kategori-mdl"] })
            toast.success(data.message || "Data kategori MDL berhasil diimpor")
            onOpenChange(false)
        },
        onError: (error: any) => {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        <DialogTitle>Import Kategori MDL</DialogTitle>
                        <DialogDescription>
                            Import data kategori MDL secara massal menggunakan file Excel (.xlsx atau .xls).
                        </DialogDescription>
                    </DialogHeader>
