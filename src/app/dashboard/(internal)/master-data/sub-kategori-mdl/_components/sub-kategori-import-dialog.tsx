...
interface SubKategoriImportDialogProps {
    open: boolean
    onOpenChange: (open: boolean) => void
}

export function SubKategoriImportDialog({ open, onOpenChange }: SubKategoriImportDialogProps) {
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Template Sub Kategori")
        XLSX.writeFile(workbook, "Template_Import_Sub_Kategori.xlsx")
        toast.success("Template Excel berhasil didownload")
    }

    const mutation = useMutation({
        mutationFn: (items: { nama: string; kode: string }[]) => SubKategoriMDLService.importSubKategori(items),
        onSuccess: (data: any) => {
            queryClient.invalidateQueries({ queryKey: ["sub-kategori-mdl"] })
            toast.success(data.message || "Data sub kategori berhasil diimpor")
            onOpenChange(false)
        },
        onError: (error: any) => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                const items = json.map((row) => {
                    const namaVal = row.nama || row.Nama || row["nama sub kategori"] || row["Nama Sub Kategori"] || ""
                    const kodeVal = row.kode || row.Kode || row["kode sub kategori"] || row["Kode Sub Kategori"] || ""
                    return {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <form onSubmit={handleSubmit}>
                    <DialogHeader>
                        <DialogTitle>Import Sub Kategori MDL</DialogTitle>
                        <DialogDescription>
                            Import data sub kategori MDL secara massal menggunakan file Excel (.xlsx atau .xls).
                        </DialogDescription>
                    </DialogHeader>
