...
import { CalendarIcon, Loader2, Check, ChevronsUpDown, ArrowRight, ArrowLeft, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react"
...
export function PengirimanFormDialog({ open, onOpenChange, pengiriman }: PengirimanFormDialogProps) {
  ...
  }, [clientItems, isEdit, pengiriman, divisions])
                ...
                {selectedItems.length > 0 && (
                  <div className="flex items-center gap-1.5 bg-neutral-100 px-3 py-1.5 rounded-md border text-xs shrink-0">
                    ...
                    <span className="font-bold text-neutral-900">
                      {Array.from(new Set(selectedItems.map(i => i.spk_number).filter(Boolean))).join(", ") || "—"}
                    </span>
                  </div>
                )}
              </div>
                      ...
                      {displayedItems.map((item) => {
                        return (
                          <tr key={item.project_item_id} className={cn("hover:bg-muted/10 transition-colors", !item.selected && "opacity-75")}>
                            ...
                            <td className="p-3 text-xs max-w-[200px] truncate text-muted-foreground">{item.deskripsi || "-"}</td>
