...
export default function PengirimanPage() {
  ...
  const [pengirimanToDelete, setPengirimanToDelete] = React.useState<
    number | null
  >(null);
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        <div>
          <h1 className='text-2xl font-bold tracking-tight pt-4 text-neutral-900'>
            Project V2 | Pengiriman
          </h1>
          <p className='text-sm text-muted-foreground'>
            Kelola data pengiriman, monitoring surat jalan, dan koordinasi
      ...
      <div className='bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden'>
        {isLoadingShipments ? (
          ...
          <div className='p-12 text-center text-muted-foreground text-sm border-neutral-200 border-dashed border-2 m-6 rounded-lg'>
            Tidak ada data pengiriman yang ditemukan. Silakan tambahkan baru
                  ...
                  >
                    ...
                    <td className='p-4'>
                      <div className='space-y-1 text-xs text-neutral-700'>
                        {item.koor_setting && (
                          <div>
                            <span className='text-neutral-400'>Koor:</span>{' '}
                            {item.koor_setting}
                          </div>
                        )}
                        {(item.tanggal_mulai_setting ||
                          item.tanggal_selesai_setting) && (
                          <div className='text-neutral-500 font-mono'>
                            {item.tanggal_mulai_setting
                              ? format(
                                  new Date(item.tanggal_mulai_setting),
                                  'dd/MM/yy'
                                )
                              : '?'}{' '}
                            -{' '}
                            {item.tanggal_selesai_setting
                              ? format(
                                  new Date(item.tanggal_selesai_setting),
                                  'dd/MM/yy'
                                )
                              : '?'}
                          </div>
        ...
        {shipmentsData && shipmentsData.last_page > 1 && (
          <div className='bg-neutral-50/50 border-t border-neutral-200 px-4 py-3 flex items-center justify-between'>
            <div className='text-xs text-neutral-500'>
              ...
              dari{' '}
              <span className='font-semibold'>{shipmentsData.last_page}</span> (
              {shipmentsData.total} total)
            </div>
            <div className='flex gap-1.5'>
              ...
              <Button
                ...
                size='sm'
                onClick={() =>
                  setPage((prev) => Math.min(prev + 1, shipmentsData.last_page))
                }
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Menghapus data pengiriman ini
              juga akan menghapus seluruh data item detail yang terkait.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
