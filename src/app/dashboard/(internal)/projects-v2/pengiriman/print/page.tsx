...
export default function PrintSuratJalanPage() {
  ...
  const [editedSetrimNo, setEditedSetrimNo] = React.useState<string | null>(null)
  ...
  const downloadExcel = () => {
    ...
    try {
      ...
      if (isSuratJalan) {
        ...
        if (pengiriman.tanggal_mulai_setting || pengiriman.tanggal_selesai_setting) {
          const tglMulai = pengiriman.tanggal_mulai_setting ? format(new Date(pengiriman.tanggal_mulai_setting), "dd MMMM yyyy") : "-"
          const tglSelesai = pengiriman.tanggal_selesai_setting ? format(new Date(pengiriman.tanggal_selesai_setting), "dd MMMM yyyy") : "-"
          const catatan = `Catatan Pemasangan / Setting:\nInstalasi dijadwalkan mulai tanggal ${tglMulai} s/d ${tglSelesai}.`
          
          wsData.push([{ v: catatan, t: 's', s: { font: { sz: 10 }, alignment: { wrapText: true } } }, '', '', '', '', '', '', '', '', '', '', ''])
          merges.push({ s: { r: wsData.length - 1, c: 0 }, e: { r: wsData.length - 1, c: 11 } })
        }

        wsData.push([])
        wsData.push([{ v: `Yogyakarta, ${format(new Date(), "dd MMMM yyyy", { locale: idLocale })}`, t: 's', s: boldStyle }, '', '', '', '', '', '', '', '', '', '', ''])
        wsData.push([])
        ...
        const defaultNoSrt = pengiriman.surat_jalan?.replace('/SJ/', '/SERTRIM/') || pengiriman.surat_jalan || "-"
        ...
        wsData.push([
          ...
          { v: ':', t: 's', s: normalStyle },
          { v: pengiriman.client?.name || "-", t: 's', s: { font: { bold: true, sz: 10 }, border: { top: borderThin, bottom: borderThin, left: borderThin, right: borderThin } } },
          '', '', '', '', '', ''
        ]) // 6 (r:5)
        ...
        wsData.push([
          ...
          { v: pengiriman.details?.[0]?.project_item?.project?.spk_number || "-", t: 's', s: { font: { bold: true, sz: 10 }, border: { top: borderThin, bottom: borderThin, left: borderThin, right: borderThin } } },
          ''
        ]) // 7 (r:6)
      ...
      XLSX.utils.book_append_sheet(wb, ws, isSuratJalan ? 'Surat Jalan' : 'Setrim')
      const fileName = `${isSuratJalan ? 'Surat_Jalan' : 'Setrim'}_${pengiriman.surat_jalan?.replace(/[\\/*?:|"<>\s]/g, "_") || pengiriman.id}.xlsx`
          ...
          <div className="space-y-3 text-[11px] mb-6 px-4">
            <div className="flex items-center">
              ...
              <div className="border border-black px-2 py-0.5 w-48 min-h-[22px] flex items-center group relative focus-within:ring-1 focus-within:ring-black">
                <input
                  type="text"
                  value={editedSetrimNo !== null ? editedSetrimNo : (pengiriman.surat_jalan?.replace('/SJ/', '/SERTRIM/') || pengiriman.surat_jalan || "")}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center shrink-0">
                <span className="w-48 font-semibold shrink-0">Tanggal Terima Barang*)</span>
                <span className="mr-2 shrink-0">:</span>
                <div className="border border-black px-2 py-0.5 w-48 min-h-[22px] shrink-0"></div>
              </div>
              <div className="flex items-center shrink-0 ml-4">
                <span className="font-semibold mr-4 text-[10px] whitespace-nowrap shrink-0">No. SPK/SPH</span>
