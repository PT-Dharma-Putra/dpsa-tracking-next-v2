...
export function MdlCatalogModal({
  ...
  // Query — ambil MDL dengan filter aktif untuk mendeskripsikan lokasi yang tersedia
  ...
  const allOnPageSelected =
    mdlList.length > 0 && mdlList.every((m) => selectedIds.has(m.id));
  const someOnPageSelected =
    mdlList.some((m) => selectedIds.has(m.id)) && !allOnPageSelected;
        ...
        <div className='bg-neutral-50/80 border-b px-6 py-3 shrink-0'>
          <div className='flex items-center gap-3 flex-wrap'>
            ...
            <Popover open={lokasiOpen} onOpenChange={setLokasiOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  role='combobox'
                  className={cn(
                    'h-9 w-[200px] justify-between bg-white border-neutral-200 text-sm font-normal',
                    selectedLokasiId === 'all' && 'text-neutral-500'
                  )}
                >
                  <span className='truncate'>
                    {selectedLokasiId === 'all' ? 'Lokasi / Ruang' : (selectedLokasiNama || 'Lokasi / Ruang')}
                  </span>
                  ...
                  <CommandList>
                    ...
                    <CommandGroup className='max-h-[220px] overflow-y-auto'>
                      <CommandItem
                        ...
                        className='text-sm'
                      >
                        <Check className={cn('mr-2 h-4 w-4', selectedLokasiId === 'all' ? 'opacity-100' : 'opacity-0')} />
                        Semua Lokasi
                      </CommandItem>
                      {lokasiOptions.map((l) => (
                        <CommandItem
                          ...
                          className='text-sm'
                        >
                          <Check className={cn('mr-2 h-4 w-4', selectedLokasiId === l.id.toString() ? 'opacity-100' : 'opacity-0')} />
                          {l.nama}
                        </CommandItem>
                      ))}
                    </CommandGroup>
            ...
            <p className='ml-auto text-xs text-neutral-500 shrink-0'>
              Total:{' '}
              <span className='font-bold text-neutral-700'>{meta.total}</span>{' '}
              data
            </p>
          </div>
        </div>
          ...
          <table className='w-full text-sm text-left border-collapse'>
            <thead className='bg-neutral-50 sticky top-0 z-10 border-b border-neutral-200'>
              <tr>
                ...
                <th className='px-4 py-3 text-xs font-bold text-neutral-500 uppercase tracking-wider'>
                  Dimensi (Meter) <br /> (P X L X T)
                </th>
                <th className='px-4 py-3 text-xs font-bold text-neutral-500 uppercase tracking-wider text-right'>
                  Harga
                </th>
                <th className='px-4 py-3 text-xs font-bold text-neutral-500 uppercase tracking-wider text-center'>
                  QTY
                </th>
              </tr>
            </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className='px-4 py-3 font-semibold text-neutral-800'>
                        ...
                        {mdl.barang?.kode && (
                          <p className='text-[10px] font-normal text-neutral-400 mt-0.5'>
                            {mdl.barang.kode}
                          </p>
                        )}
                      </td>
                      <td className='px-4 py-3 text-xs text-neutral-600 max-w-45'>
                        {mdl.barang?.spesifikasi ? (
                          <span
                            className='line-clamp-2'
                            title={mdl.barang.spesifikasi}
                          >
                            {mdl.barang.spesifikasi}
                          </span>
                        ) : (
                      ...
                      >
                        <input
                          type='number'
                          min={1}
                          value={qtyMap[mdl.id] ?? 1}
                          onChange={(e) => {
                            const val = Math.max(1, parseInt(e.target.value) || 1);
                            setQtyMap((prev) => ({ ...prev, [mdl.id]: val }));
                          }}
                          className='w-16 h-7 text-center text-xs border border-neutral-200 rounded bg-white focus:outline-none focus:ring-1 focus:ring-orange-400'
                        />
                      </td>
                    </tr>
                  );
        ...
        {meta.last_page > 1 && (
          <div className='bg-white border-t px-6 py-3 flex items-center justify-between shrink-0'>
            <p className='text-xs text-neutral-500'>
              ...
              dari <span className='font-bold'>{meta.last_page}</span>
            </p>
