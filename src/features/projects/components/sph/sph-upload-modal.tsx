...
export function SPHUploadModal({ open, onOpenChange, projectId, onSuccess }: SPHUploadModalProps) {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            queryClient.invalidateQueries({ queryKey: ['project-sph', projectId] })
            onOpenChange(false)
            onSuccess?.()
        } catch (error) {
            console.error(error)
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Upload Final SPH</DialogTitle>
                    <DialogDescription>
                        Upload the finalized PDF Quotation document. Client will be able to see this file.
                    </DialogDescription>
                </DialogHeader>
